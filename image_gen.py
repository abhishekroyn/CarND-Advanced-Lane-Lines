...
# images = glob.glob('./debug_dir/debug_frames/temp/video_frame_*.jpg')     # for plotting durinig DEBUGGING only
...
#    res_yvals = np.arange(warped.shape[0]-(window_height/2), 0, -window_height)
    ...
    inner_lane = np.array(list(zip(np.concatenate((left_fitx+window_width/2, right_fitx[::-1]-window_width/2), axis=0), np.concatenate((yvals, yvals[::-1]), axis=0))), np.int32)    # middle_marker
